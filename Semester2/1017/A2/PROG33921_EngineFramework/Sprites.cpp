...
void Player::SetDir(int dir){ // Will be used to set direction of sprite. Just added it for you.
	m_iDir = dir;
}
...
void Player::Update(){
	...
	m_dVelY += m_dAccelY + m_dGrav / 6;
	m_dVelY = min(max(m_dVelY, -(m_dMaxVelY * 10)), (m_dMaxVelY));
	m_rDst.y += (int)m_dVelY;
}

Obstacle::Obstacle(int x, bool b, SDL_Rect src,	SDL_Rect dst, bool p, bool r){
	m_iX = x;
	if (b){
