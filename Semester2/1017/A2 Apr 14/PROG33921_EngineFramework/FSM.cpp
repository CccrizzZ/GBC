...
void GameState::Enter(){
	...
	m_pPlayer = new Player(
		{0,0,0,0},   							// Src Rect
		{1024/2-64,300,128,128}		// Dst Rect
	);

	m_pGround = new Obstacle(
		0,
		true,
		{0,0,0,0},
		{0,512,1024,256},
...
void GameState::Update(){
	...
	if (Game::Instance()->KeyDown(SDL_SCANCODE_SPACE) && Game::Instance()->GetSpaceOk() && m_pPlayer->IsGrounded()){
		Game::Instance()->SetSpaceOk(false);
		m_pPlayer->SetAccelY(-GRAV * 2.5); // Sets the jump force.
...
void GameState::Render(){
	...
	SDL_RenderFillRect(Game::Instance()->GetRenderer(), m_pPlayer->GetDstP());
